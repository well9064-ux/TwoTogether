...
test("접근성에 필요한 문서 구조와 대체 입력을 제공한다", async () => {
  ...
  assert.match(page, /setMileageBalance\(\(balance\) => balance - 300\)/);
  ...
  assert.match(page, /setMileageBalance\(\(balance\) => balance \+ 240\)/);
  assert.match(page, /신청 비용의 80%인 240 마일리지가 반환됩니다/);
